...
Covers:
  ...
  - A03 Actor Injection: assert_safe_actor rejects dangerous actor names
...
from src.esaa.utils import assert_safe_actor, assert_within_root, normalize_rel_path
...
class TestSubmitPathTraversal:
    ...
    def test_submit_refuses_escape_path(self, initialized_project: Path) -> None:
        ...
        task = next(t for t in roadmap["tasks"] if t.get("status") == "todo")
        task_id = task["task_id"]

        svc.claim_task(task_id, actor="agent-test")
